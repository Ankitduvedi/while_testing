...
class SplashRepository {
  ...
  Future<Either<Failure, bool>> intializeWhile() async {
    try {
      ...
      debugPrint("version number is $versionNumber and review version number is $reviewVersionNumber");
  ...
  Future<String> getAppVersion() async {
    try {
      final data = _firestore.collection("versions").doc(Platform.isAndroid ? "Android" : "iOS");
  ...
  Future<Either<Failure, int>> checkCondition() async {
    try {
      final tempaccessToken = await SecureStorage().getUserAccessToken('tempAccessToken');
      if (tempaccessToken != 'onboarded') {
        return right(1);
      } else {
        final accessToken = await SecureStorage().getUserAccessToken('accessToken');
        if (accessToken != '') {
          return right(2);
        } else {
          return right(3);
